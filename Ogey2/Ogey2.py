...
class EnterenceTube(pygame.sprite.Sprite):
    '''Class for creating Enterence Tube'''
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("world/LevelExit.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]


class ExitTube(pygame.sprite.Sprite):
    '''Class for exit tube'''
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("world/PipeEntry.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]
...
class CleanPlatform(pygame.sprite.Sprite):
    '''Class for creating long gray platforms'''
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("world/CleanPlatform.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]
...
class Wall(pygame.sprite.Sprite):
    ...
    def __init__(self, x, y):
        ...
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("world/JumpObstacle.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]

class SmallPlatform(pygame.sprite.Sprite):
    '''Class for creating small gray platforms'''
    def __init__(self, x, y):
        self.x = x
        self.y = y
        pygame.sprite.Sprite.__init__(self)
        self.image = pygame.image.load("world/ChallengePlatform.png").convert_alpha()
        self.rect = self.image.get_rect()
        self.rect.topleft = [self.x, self.y]
        ...
        for row in self.level1:
            for col in row:
                ...
                if col == "W":
                    ...
                    self.all_sprite.add(self.world)
                if col == "E":
                    exitTube = ExitTube(x, y)
                    self.world.append(exitTube)
                    self.all_sprite.add(self.world)
                if col == "A":
                    enterenceTube = EnterenceTube(x, y)
                    self.world.append(enterenceTube)
                    self.all_sprite.add(self.world)
                if col == "M":
                    poop = Poop(x, y)
                    ...
                    self.all_sprite.add(self.world)
                if col == "Y":
                    clean = CleanPlatform(x, y)
                    self.world.append(clean)
                    self.all_sprite.add(self.world)
                if col == "K":
                    challenge = SmallPlatform(x, y)
                    self.world.append(challenge)
                    self.all_sprite.add(self.world)
                if col == "S":
                    spikes = Spikes(x, y)
